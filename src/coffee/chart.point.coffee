class Chart.Point extends Base.Linear
  ...
  drawItem: (data, index) ->
    ...
    cy = @margins.top + @drawable_area_height * factorY
    ...
    if index is @data.length - 1
      ...
      end_zero_x = (@margins.left + @drawable_area_width) * @real_width / 100
      ...
      @appendUIElement path
      @points = []


  # Sets width of the bar
  _setItemAnchorSize: ->
    @item_anchor_size = @drawable_area_width / (@data.length - 1)
